...
import { actionVinhoGetDeliveryman } from "store/actions";

export const getDeliveryman = async () => {
  const resp = await api.get("/deliveryman");
  const products = resp.data;
  store.dispatch(actionVinhoGetDeliveryman(products));
  return products;
};

export const getDeliverymanGroupedByCategory = async () => {
  let resp = [];
  await api
...
export const getDeliverymanByCategory = async (category) => {
...
export const getDeliverymanPorNome = async (nome) => {
